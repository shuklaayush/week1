...
template sudoku() {
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            ...
            assert(puzzleRangeProof[i][j].out); //[assignment] change assert() to use your created RangeProof instead
            ...
            assert(solutionRangeProof[i][j].out); //[assignment] change assert() to use your created RangeProof instead
